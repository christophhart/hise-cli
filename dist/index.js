...
function startSpinner(msg = "") {
    ...
    spinnerInterval = setInterval(() => {
        ...
        if (lastProgressValue !== null) {
            ...
            const frame = chalk.yellow(SPINNER_FRAMES[spinnerFrame % SPINNER_FRAMES.length]);
            process.stdout.write(`${frame} ${chalk.dim(spinnerText)}`);
        }
        spinnerFrame++;
    }, 80);
}
function updateSpinner(msg, progress) {
    spinnerText = msg;
...
// REPL loop
// ---------------------------------------------------------------------------
async function runRepl(pipeName) {
    let socket;
    try {
